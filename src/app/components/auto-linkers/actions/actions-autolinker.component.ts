...
@Component({
    // moduleId: module.id,
    selector: 'content-component-actions-autolinker',
    templateUrl: 'actions-autolinker.component.html?v=${new Date().getTime()}',
    styleUrls: [ 'actions-autolinker.component.css?v=${new Date().getTime()}' ],
    providers: [HTTPService]
})

export class ActionsAutolinkerComponent {
    id: any;
    storage_url:string;
    private sub: any;
    public ItemData:any = {
    ...
    saveItem(e){
        ...
        this._httpService.postData( 'article-anchors'+id , data).subscribe(
    ...
    checkAction(){
        if (this.id == 'add') {
            ...
            this.router.navigate(['anchors/'+ itm.id]);
        }else{
            this.generateItem()
        }
    }
    ...
    getItem(){
        this._httpService.getData('article-anchors/'+this.id).subscribe(
            data => this.ItemData = data,
            error => alert(error),
            () => this.generateItem()
        );
